...
public class Fibonacci {
	...
	public static int atIndex(int i) throws IllegalArgumentException {
		...
		int calculatedElement = fibonacci[i];
	...
	public static int[] slice(int start, int end) throws IllegalArgumentException {
		...
		int[] rawFibonacciArray = fibonacci(end + 1);
		int lengthOfCalSlice = end - start;
		int[] calculatedSlice = new int[lengthOfCalSlice];

		for (int i = 0; i < lengthOfCalSlice; i++, start++) {
			calculatedSlice[i] = rawFibonacciArray[start];
		}
	...
	public static int[] fibonacci(int count) throws IllegalArgumentException {
		...
		int[] numSeq = new int[count + 2];
		int[] numSeq2 = new int[1];
		if (count > 1) {
			int x = 0;
			int y = 1;
			numSeq[x] = 1;
			...
			for (int i = 0; i < count; i++, x++, y++) {
				numSeq[y + 1] = numSeq[x] + numSeq[y];
			}
			numSeq2 = new int[numSeq.length - 2];
			System.arraycopy(numSeq, 0, numSeq2, 0, numSeq2.length);
			return numSeq2;
		} else if (count == 1) {
			numSeq2[0] = 1;
			return numSeq2;
